...
public class RelevantFactorRVAdapter extends ClassicRecyclerViewAdapter<RelevantFactor> {

    public RelevantFactorRVAdapter(List<RelevantFactor> mDataList, int mItemLayoutId) {
    ...
    public void findBindView(final int position, ClassicRecyclerViewHolder vHolder) {
        ...
        tv_name.setText(mDataList.get(position).YSMS);
        tv_name.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                if (!EmptyTool.isBlank(s.toString())) {
                    mDataList.get(position).YSMS = s.toString();
