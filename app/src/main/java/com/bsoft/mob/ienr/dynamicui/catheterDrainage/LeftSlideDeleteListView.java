package com.bsoft.mob.ienr.dynamicui.catheterDrainage;
...
public class LeftSlideDeleteListView extends ListView {
    ...
    private class SlideContainer extends FrameLayout {
        ...
        public SlideContainer(Context context, View topChild) {
            ...
            //用于包裹删除按钮
            LinearLayout root = LayoutParamsHelper.buildLinearMatchWrap_V(context);
//            root.setLayoutParams(new LinearLayout.LayoutParams(LinearLayout.));
            root.setGravity(Gravity.RIGHT | Gravity.CENTER_VERTICAL);
            root.setBackgroundResource(R.color.red);
            delete = new ImageView(context);
            Drawable drawable = ContextCompatHelper.getDrawable(context, R.drawable.ic_delete_forever_black_24dp,R.color.white);
            delete.setImageDrawable(drawable);
            int padding = dp2px(getContext(), 30);
