...
public class JsonUtil {
	...
	private static ObjectMapper generateMapper(Inclusion inclusion) {
		...
		//
		// // 所有日期格式都统一为以下样式
		// customMapper.setDateFormat(new
		// SimpleDateFormat("yyyy-MM-dd HH:mm:ss"));

		// 允许单引号
		customMapper.configure(Feature.ALLOW_SINGLE_QUOTES, true);
		// 允许key不带引号
		customMapper.configure(Feature.ALLOW_UNQUOTED_FIELD_NAMES, true);
		// 将emun解析为数字
		customMapper.configure(SerializationFeature.WRITE_ENUMS_USING_INDEX,
				true);
		// 将单个对象将为数组
